...
class LinearPower(object):
    ...
    def __init__(self, cosmo, redshift, transfer='CLASS'):
        ...
        # setup the transfers
        if transfer not in ['CLASS', 'NoWiggleEisensteinHu', 'EisensteinHu']:  # Use the actual class names here
        ...
        c = Class()
        c.set(pars)
        c.compute()# transfers get an internal copy
        ...
        # fallback to analytic when out of range
    ...
    def calc_sigma_r(self, filter_radius=8., kmin=1e-5, kmax=1e0):

        k = np.logspace(np.log10(kmin), np.log10(kmax), 1024)
        
        # Get the linear power spectrum at a specific redshift (e.g., z=0)
        Plin = self(k)
        ...
        # Numerical integration to get the variance
